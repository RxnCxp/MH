...
function renderItems(itemsData, section, searchTerm) {
  ...
  for (let i = 0; i < itemsData.length; i += itemsPerRow) {
    ...
    for (let j = i; j < i + itemsPerRow && j < itemsData.length; j++) {
      ...
      const isElderDragon =
        section === "monsters" && item.species === "elder dragon";
      ...
      if (shouldShowItem) {
        const itemHtml = `
            <div class="card ${isLargeMonster ? "large-monster" : ""} ${
          isElderDragon ? "elder-dragon" : ""
  ...
  switch (section) {
    case "ailments":
      return `
        ...
        <p>Recovery Actions: ${
          item.recovery ? item.recovery.actions : "N/A"
        }</p>
    ...
    case "armor":
      return `     
        ...
        <p>Fire Resistance: ${
          item.resistances ? renderStars(item.resistances.fire) : "N/A"
        }</p>
        <p>Water Resistance: ${
          item.resistances ? renderStars(item.resistances.water) : "N/A"
        }</p>
        <p>Ice Resistance: ${
          item.resistances ? renderStars(item.resistances.ice) : "N/A"
        }</p>
        <p>Thunder Resistance: ${
          item.resistances ? renderStars(item.resistances.thunder) : "N/A"
        }</p>
        <p>Dragon Resistance: ${
          item.resistances ? renderStars(item.resistances.dragon) : "N/A"
        }</p>
